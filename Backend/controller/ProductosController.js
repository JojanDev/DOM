...
import ProductosServicios from "../Services/Productos.js";
...
class ProductosController {
  ...
  static getProductosByID = async (req, res) => {
    try {
      const { id } = req.params;
      const objProducto = new Producto();
      const productos = await objProducto.getByID(id);
  ...
  static getProductosByIdCategoria = async (req, res) => {
    try {
      const { id } = req.params;
      const objProducto = new ProductosServicios();
      const productos = await objProducto.getByIdCategoria(id);
      res.status(200).json(productos);
    } catch (error) {
      res.status(500).json({ error: error.message });
    }
